...
@php
    $businessTypeModel = $businessTypes->firstWhere('slug', request('business_type'));
    $businessTypeName = $businessTypeModel->name ?? request('business_type');
    $businessTypeIcon = $businessTypeModel->icon ?? null;
    $businessTypeDescription = $businessTypeModel->description ?? null;
@endphp

@section('title', $businessTypeName . ' - ' . config('app.name'))

@section('meta')
    <meta name="description" content="Browse {{ $businessTypeName }} across Nigeria. Find verified and trusted {{ strtolower($businessTypeName) }} near you.">
    <meta name="keywords" content="{{ strtolower($businessTypeName) }}, {{ strtolower($businessTypeName) }} Nigeria, local {{ strtolower($businessTypeName) }}">
@endsection
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4 mb-4">
            @if($businessTypeIcon)
                <span class="text-5xl">{{ $businessTypeIcon }}</span>
            @endif
            <div>
    ...
    <nav class="flex mb-6 text-sm text-gray-600 dark:text-gray-400">
        ...
        @if(request('state'))
            ...
            <span class="mx-2">/</span>
        @endif
        <span class="text-gray-900 dark:text-white">{{ $businessTypeName }}</span>
    </nav>

    <!-- Active Filters Display -->
    @if(request()->except(['page', 'per_page', 'business_type'])->count() > 0)
        <div class="mb-6 flex flex-wrap items-center gap-2">
            ...
            <a href="/{{ request('business_type') }}" class="text-sm text-blue-600 dark:text-blue-400 hover:underline">
                Clear all
            </a>
        </div>
    @endif
        ...
        <div class="flex-1">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a 
                        href="{{ route('discover.index', array_filter(['business_type' => request('business_type'), 'category' => request('category'), 'state' => request('state'), 'city' => request('city')])) }}"
            ...
            @else
                ...
                <div class="text-center py-16">
                    ...
                    <h3 class="mt-4 text-xl font-medium text-gray-900 dark:text-white">No {{ strtolower($businessTypeName) }} found</h3>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">
                        Try adjusting your filters or check back later
                    </p>
                    <a href="/{{ request('business_type') }}" class="mt-4 inline-block px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                        Clear Filters
                    </a>
                </div>
            @endif
